...
3. Expose it with Portal: `portal expose 127.0.0.1:7466 --name <dns-label-safe-name>` with an absolute `--identity-path` outside any repository (the default `identity.json` in the working directory contains private key material — never commit or print it). Mention that the hostname is publicly listed on participating relays unless the user wants `--hide`. Never add TCP, UDP, multi-hop, or payment flags the user did not request.
