...
3. Expose it with Portal: `portal expose 127.0.0.1:7466 --name <dns-label-safe-name> --hide` with an absolute `--identity-path` outside any repository (the default `identity.json` in the working directory contains private key material — never commit or print it). Listing has no value for a personal collab relay, and the relay accepts connections from anyone anyway — the room keys are the real boundary. Never add TCP, UDP, multi-hop, or payment flags the user did not request.
